  ...
  <div class="max-w-6xl mx-auto space-y-8 sm:space-y-12">
    ...
    <nav class="flex items-center justify-between gap-1.5 sm:gap-2 rounded-full bg-[#ebebeb] dark:bg-muted px-3.5 sm:px-10 py-2 sm:py-3">
      ...
      <div class="flex items-center gap-1.5 sm:gap-4 min-w-0">
        <span class="hidden sm:inline rounded-full bg-card font-bold text-primary text-lg px-5 py-2">Infaq &amp; Shodaqoh</span>

        <label class="relative flex items-center rounded-full bg-card min-w-0">
          <span class="sr-only">Pilih jenis infaq</span>
          <select
            v-model="selectedId"
            class="appearance-none bg-transparent bg-none border-0 rounded-full font-bold text-primary text-sm sm:text-lg cursor-pointer focus:outline-none focus:ring-0 focus-visible:ring-2 focus-visible:ring-ring truncate pl-3 sm:pl-5 pr-7 sm:pr-12 py-1.5 sm:py-2 sm:min-w-56"
          >
    ...
    <div class="grid grid-cols-1 md:grid-cols-12 gap-8 lg:gap-16 items-start max-w-5xl mx-auto">
      <div class="md:col-span-7 space-y-6 sm:space-y-8 text-foreground text-base sm:text-xl leading-relaxed">
        ...
        >
          <Smartphone class="w-8 h-8 text-amber-600 dark:text-amber-400 shrink-0" />
