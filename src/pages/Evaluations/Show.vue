<template>
  <div class="max-w-6xl mx-auto space-y-6">
    ...
    <!-- ================================================================= -->
    <div v-else-if="evaluation && userEvaluation && userEvaluation.finished_at" class="space-y-6">
      <h2 class="text-xl sm:text-2xl font-bold text-foreground text-center">{{ evaluation.title }}</h2>
        ...
        >
          ...
          <div v-else class="py-1 divide-y divide-border/60">
            <div
              ...
              :key="option"
              :class="[
                userEvaluation.answers && userEvaluation.answers[question.id] === option
...
// Finished evaluations are always opened so their results stay viewable.
...
const lockTitle = computed(() => {
  ...
  return 'Evaluasi Tidak Dapat Dikerjakan';
});
...
const answeredCount = computed(() => {
  if (!evaluation.value?.questions) return 0;
  return evaluation.value.questions.filter((q) => selected.value[q.id] !== undefined && selected.value[q.id] !== '').length;
});
...
async function loadData(refreshList = false) {
  ...
  try {
    ...
    if (!data.success) {
      blockedMessage.value = data.message || 'Evaluasi ini tidak dapat dikerjakan saat ini.';
      return;
    }
    ...
    blockedMessage.value = 'Terjadi kesalahan saat memuat evaluasi.';
  } finally {
...
async function goToEvaluation(position) {
  ...
  if (userEvaluation.value && !userEvaluation.value.finished_at) {
    const res = await swConfirm({
      title: 'Tinggalkan Evaluasi?',
      text: 'Waktu evaluasi akan terus berjalan jika Anda pindah. Yakin ingin membuka evaluasi lain?',
      confirmButtonText: 'Ya, Pindah',
      cancelButtonText: 'Lanjutkan Evaluasi',
      icon: 'warning',
    });
    if (!res.isConfirmed) return;
  }
  router.push({ name: 'evaluations.show', params: { id: target.id } });
}
...
async function confirmSubmit() {
  ...
  const res = await swConfirm({
    ...
    text: isAllAnswered
      ? 'Semua soal telah Anda jawab. Apakah Anda yakin ingin menyelesaikan evaluasi sekarang?'
      : `Masih ada ${unanswered} soal yang belum dijawab. Apakah Anda yakin ingin mengakhiri dan mengumpulkan evaluasi sekarang?`,
...
async function submitAnswers() {
  ...
  try {
    ...
    await swAlert({
      ...
      text: 'Terjadi kesalahan saat mengumpulkan evaluasi. Silakan coba lagi.',
      buttonText: 'OK',
    });
  } finally {
